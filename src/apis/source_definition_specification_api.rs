...
pub async fn get_source_definition_specification(configuration: &configuration::Configuration, source_definition_id_with_workspace_id: crate::models::SourceDefinitionIdWithWorkspaceId) -> Result<crate::models::SourceDefinitionSpecificationRead, Error<GetSourceDefinitionSpecificationError>> {
...
pub async fn get_specification_for_source_id(configuration: &configuration::Configuration, source_id_request_body: crate::models::SourceIdRequestBody) -> Result<crate::models::SourceDefinitionSpecificationRead, Error<GetSpecificationForSourceIdError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v1/source_definition_specifications/get_for_source", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetSpecificationForSourceIdError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
